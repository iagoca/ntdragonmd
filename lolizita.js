...
if (mek.key.fromMe) return
if(isGroupAdmins) return lolizita.sendMessage(from,{text: 'mensagem proibida detectada, porém é admin logo a punição será anulada'}, {quoted: mek})
...
case 'ping':
addFilter(from)
  sendButMessage(from,`💠𝑉𝑒𝑙𝑜𝑐𝑖𝑑𝑎𝑑𝑒 𝑑𝑒 𝑟𝑒𝑠𝑝𝑜𝑠𝑡𝑎:  ${latensi.toFixed(4)}💬\n\n💠𝑇𝑒𝑚𝑝𝑜 𝑎𝑡𝑖𝑣𝑜:  ${runtime(process.uptime())} ⏳\n`,`𝐍𝐓 𝐃𝐑𝐀𝐆𝐎𝐍`,
  [
              {              
                buttonId: `ping`,
                buttonText: {
...
if (args.length < 1) return enviar('Ensira [on] pra ativar e [off] pra desativar\n\nExemplo: antifoto on');
if (args[0] === 'on') {
if (isAntiImg) return enviar("O anti foto já está ativado.");
antiimg.push(from)
...
if (args[0] === 'on') {
if (isAntiAudio) return enviar("O anti áudio já está ativado.");
antiaudio.push(from)
...
execute(`ffmpeg -i ${media} -vf scale=512:512 ${ran}`, async function(err, result) {
if(err) return enviar("Err 186")
console.log('⏐\n⏐\n⏐\n⏐Enviando figu automática de 🥀', color(pushname,'green'), '🥀\n⏐\n⏐\n⏐')
await lolizita.sendMessage(from, {sticker: {url: `./${ran}`}}, {quoted: mek})
await fs.unlinkSync(media)
await fs.unlinkSync(ran)
})
...
addFilter(from)
		...
		break
          ...
          addFilter(from)
              ...
              await sendFileFromUrl(from,waifu.url,` Olá ${pushname} prontinho`,mek)
              }
		break
...
case 'bot':
pre = `👋🏻Olá @${sender.split('@')[0]} me chamo NT DRAGON-MD,  sou um bot->(robô) estou aqui para lhe entreter e ajudar em algumas tarefas, para ver meus comandos mande❗ MENU ou M ❗.\n\nNão é preciso utilizar prefixo mas se usar não tem problema 😁💙\n\n\n[Se divirta com meus comandos !]`
enviar(String(pre))
break
...
let teks = `══✪〘 *👥 Atenção!* 〙✪══
 ...
 ➲ Mensagem: ${q ? q : 'Nada não'}\n\n`
	   ...
	   case 'play':{
	   addFilter(from)
		   if (!q) return enviar ('🙄Nao econtro músicas que não tem nome🙄 ')
...
yhb = `Que fofo... @${sender.split('@')[0]}  deu um abraço😍 bem apertado no(a) @${mentioned[0].split('@')[0]} 🤗`
enviar (`${yhb}`)
break

case 'regras': {
addFilter(from)
enviar ('☣️Regras basicas do bot ☣️\n\n⬇️[1] Ligar para o bot (block)️\n⬇️\n⬇️[2] Adicionar e remover em massa (ban permanente)️\n⬇️\n⬇️[3]Spam/flood (ban permante)\n⬇️\n⬆️️[4] Desrespeito ao criador do bot (block)')
}
break
...
lolizita.sendMessage(from, {image: gambar, caption: `Prontinho vida❤️`, quoted : mek})
})                    
break
...
case 'hora':
addFilter(from)
enviar (`𝐻𝑜𝑟𝑎𝑟𝑖𝑜 𝑑𝑒 𝐵𝑟𝑎𝑠𝑖𝑙𝑖𝑎: ${hr}`)
break

case 'qrcode':
addFilter(from)
if (!q) return lolizita.sendMessage(from, {text: 'Digite um texto/url que deseja criar um código qr'}, {quoted: mek})
